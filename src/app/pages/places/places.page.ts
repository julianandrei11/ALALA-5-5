...
export class PlacesPage implements OnInit, OnDestroy {
  ...
  async deleteCurrentCard() {
    ...
    const ok = await this.confirmService.confirm({
      title: 'Delete place?',
      message: `Remove “${this.currentCard.label || 'this place'}”? This can’t be undone.`,
      confirmText: 'Delete',
      cancelText: 'Cancel',
      tone: 'danger'
    });
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('card-deleted', { detail: { cardId: deletedId, category: 'places' } })
      );
      await this.confirmService.notify('Place was deleted.', 'Deleted');
    } catch (err) {
      console.error('Failed to delete card:', err);
      await this.confirmService.notify('Could not delete this place. Please try again.', 'Couldn’t delete');
    }
  }
  ...
  async saveCardEdit() {
    ...
    try {
      ...
      const storageKey = `placesCards_${uid}`;
      ...
      this.cdr.detectChanges();
      await this.toast('Place updated successfully', 'success');
      this.closeEditModal();
